...
const handlePermissionsByRole = () => {
  if (formData.value.roles.includes(ROLE.ADMIN)) {
    formData.value.permissions = {...adminPermissions.value}
  } else if (formData.value.roles.includes(ROLE.EDITOR)) {
    formData.value.permissions = {...editorPermissions.value}
  } else {
    formData.value.permissions = {...defaultPermissions.value}
  }
}
const addAccount = async () => {
  try {
    await userStore.addAccount(formData.value);
    await router.push({name: 'account_cms'})
  } catch (error) {
    errors.value = {...defaultErrors.value,...error.response.data.errors}
  }
}
onMounted(async () => {
  ...
  const fullValidation = new Validation(fullValidationForm, {
    submitCallback: async (e, valid) => {
      if (valid) {
        ...
        //   el.innerHTML = error.response.data.errors.email.toString()
  ...
  <div class="w-full">
    <form id="formAccountAdd">
      <div class="grid grid-cols-12 gap-4">
        ...
        <div class="col-start-4 col-span-8 text-sm text-red-600" v-show="errors.email.length > 0">
        ...
        <div class="col-span-3">
          <input class="checkbox cursor-pointer" type="checkbox" :value="ROLE.ADMIN" id="roleAdminCheckbox"
                 v-model="formData.roles" @change="handlePermissionsByRole"/>
          <label
        ...
        <div class="col-span-6">
          <input class="checkbox cursor-pointer" type="checkbox" :value="ROLE.EDITOR" id="roleEditorCheckbox"
                 v-model="formData.roles" @change="handlePermissionsByRole"/>
          <label
              class="inline-block pl-[0.15rem] cursor-pointer"
        ...
        <div class="col-start-4 col-span-8 text-sm text-red-600" v-show="errors.roles.length > 0">
          {{ errors.roles.toString() }}
        </div>
        <div class="col-span-3 row-span-3 flex items-start">
          Permissions
        </div>
        <template v-for="(permissionGroup, key) in formData.permissions">
              ...
              <div class="mb-2"
                ...
                <div class="mb-[0.125rem] mr-4 inline-block min-h-[1.5rem] pl-[1.5rem]"
                  ...
                  <input
                      ...
                      type="checkbox"
                      :checked="value"
                      v-model="formData.permissions[key][key2][permission]"
            ...
            <div v-else class="flex">
              <div class="mb-[0.125rem] mr-4 inline-block min-h-[1.5rem] pl-[1.5rem]"
                ...
                <input
                    ...
                    type="checkbox"
                    :checked="value"
                    v-model="formData.permissions[key][permission]"
                    id="permissionCheckbox"
                />
                <label
                    class="inline-block pl-[0.15rem] hover:pointer-events-none"
