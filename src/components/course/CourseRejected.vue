...
onMounted(async () => {
  ...
  const response = await courseStore.getListRejected()
  ...
  data.value.rows = courseStore.listRejected.map((course, index) => {
...
async function getListRejected() {
  const response = await courseStore.getListRejected(helper.toQueryString(formData.value))
...
<template>
  ...
  <Pagination :currentPage="pagination.currentPage" :total="pagination.total" :perPage="pagination.perPage"
              @change="(page) => {formData.page = page; getListRejected();}"
              @changePerPage="(perPage) => {formData.perPage = perPage; getListRejected()}"/>
