...
onMounted(async () => {
  ...
  const response = await courseStore.getListApproved()
  ...
  data.value.rows = courseStore.listApproved.map((course, index) => {
  ...
  const setActions = () => {
    document.querySelectorAll(".edit-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        router.push({name: 'course_detail', params: {type: 'approved', id: btn.attributes["data-te-id"].value}, props: true})
      });
    });
  };
...
<template>
  ...
  <Pagination :currentPage="pagination.currentPage" :total="pagination.total" :perPage="pagination.perPage"
              @change="(page) => {formData.page = page; getListApproved();}"
              @changePerPage="(perPage) => {formData.perPage = perPage; getListApproved()}"/>
