...
const exercisesSeleted = computed(() => {
  return exercisesStore.exercises.find((item) => item.id == route.query?.idExercises)
})
...
watch(
  () => exercisesSeleted.value,
  () => {
    formData.id = exercisesSeleted.value?.id
    formData.lesson_detail_id = exercisesSeleted.value?.lesson_detail_id
    formData.language = exercisesSeleted.value?.language
    formData.content = exercisesSeleted.value?.content
    formData.status = exercisesSeleted.value?.status
  },
...
async function updateExercisesBtn() {
  // check validate
  const result = await v$.value.$validate()
  if (result) {
    exercisesStore.updateExercises(formData)
    router.push({
      name: 'exercises_view',
